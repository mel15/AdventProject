...
def main():
    ...
    for iteration in tqdm(range(10000)):
        ...
        total_safety = calculate_quadrants_safety(board)

        if total_safety < 50_000_000:
            print(f"iteration {iteration + 1}")
            print_board(board)
            print(f"Total safety is {total_safety}")
            break

    print(f"Iteration tree is {iteration + 1}")


def print_board(board):
    in_line = [length_line - line.count(".") for line in board]
    greater_than = [value for value in in_line if value > 25]
